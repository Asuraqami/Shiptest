...
/obj/machinery/computer/helm/ui_data(mob/user)
	...
	// ARPA – sensor contacts
	.["arpa_ships"] = list()
	var/list/arpobjects = current_ship.check_proximity()
	for(var/datum/overmap/ship/controlled/object in arpobjects)
		if(!istype(object, /datum/overmap/ship/controlled))   // filter non‑ships
			continue
		var/list/cpa_list = calculate_cpa(current_ship, object, TRUE)
		.["arpa_ships"] += list(list(
	...
	// Radar – contacts including docked ships (dynamic while‑loop)
	.["otherInfo"] = list()
	...
	while (dequeue_pointer++ < objects.len)
		var/datum/overmap/object = objects[dequeue_pointer]
		...
		if(istype(object, /datum/overmap/ship/controlled))
			var/datum/overmap/ship/controlled/controlled_object = object
			if(controlled_object.shuttle_port)
				for(var/obj/docking_port/stationary/docking_port as anything in controlled_object.shuttle_port.docking_points)
					if(current_ship.shuttle_port.check_dock(docking_port, silent = TRUE, intention_to_dock = FALSE))
						available_dock = TRUE
						break

		objects |= object.contents   // append docked ships so they are processed later
...
/obj/machinery/computer/helm/ui_act(action, list/params, datum/tgui/ui, datum/ui_state/state)
	...
	switch(action)
		...
		if("send_sos")
			if(!current_ship.SendSos(name = "[current_ship.name]", x = "[current_ship.x || current_ship.docked_to.x]", y = "[current_ship.y || current_ship.docked_to.y]"))
				if(COOLDOWN_TIMELEFT(current_ship, sendsos_cooldown)/10 != 0)
					say("Error: [COOLDOWN_TIMELEFT(current_ship, sendsos_cooldown)/10] seconds until S.O.S. signal recharge.")
				return
		...
		if("act_overmap")
			if(SSshuttle.jump_mode > BS_JUMP_CALLED)
				to_chat(usr, "<span class='warning'>Cannot interact due to bluespace jump preparations!</span>")
				return
	...
	if(!current_ship.docked_to && !current_ship.docking)
		switch(action)
			...
			if("quick_dock")
				if(SSshuttle.jump_mode > BS_JUMP_CALLED)
					to_chat(usr, span_warning("Cannot dock due to bluespace jump preparations!"))
					return
		...
		if(action == "undock")
			...
			if(current_ship.avg_fuel_amnt < 25 && tgui_alert(usr, "Ship only has ~[round(current_ship.avg_fuel_amnt)]% fuel remaining! Are you sure you want to undock?", "Undock Confirmation", list("Yes", "No")) != "Yes")
				return
			current_ship.Undock()
